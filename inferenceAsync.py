...
async def createInstance(sadtalker_paths, device):
    ...
    animate_from_coeff = animate_from_coeff_t.result()
    instances = [preprocess_model, audio_to_coeff, animate_from_coeff ]
...
def main(args):
    ...
    startTime = time.time()
    instances = asyncio.run(createInstance(sadtalker_paths, device))
    preprocess_model = instances[0]
    audio_to_coeff = instances[1]
    animate_from_coeff = instances[2]
    
    endTime = time.time()    
    execution_time = endTime - startTime
    print(f"3 Class Execution Time in parallel: {execution_time} seconds")
