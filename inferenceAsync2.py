...
async def main(args):
    ...
    sadtalker_paths = await init_path(args.checkpoint_dir, os.path.join(current_root_path, 'src/config'), args.size, args.old_version, args.preprocess)
    ...
    # preprocess_model = await CropAndExtract(sadtalker_paths, device)
    ...
    # audio_to_coeff = await Audio2Coeff(sadtalker_paths,  device)
    ...
    # animate_from_coeff = await AnimateFromCoeff(sadtalker_paths, device)
    ...
    first_coeff_path, crop_pic_path, crop_info = await preprocess_model.generate(pic_path, first_frame_dir, args.preprocess,\
    ...
    if ref_eyeblink is not None:
        ...
        ref_eyeblink_coeff_path, _, _ = await preprocess_model.generate(ref_eyeblink, ref_eyeblink_frame_dir, args.preprocess, source_image_flag=False)
    else:
    ...
    if ref_pose is not None:
        if ref_pose == ref_eyeblink: 
            ...
            ref_pose_coeff_path, _, _ = await preprocess_model.generate(ref_pose, ref_pose_frame_dir, args.preprocess, source_image_flag=False)
    else:
    ...
    #audio2ceoff
    generate_startTime = time.time()
    batch = await get_data(first_coeff_path, audio_path, device, ref_eyeblink_coeff_path, still=args.still)
    generate_endTime = time.time()    
    execution_time = generate_endTime - generate_startTime
    ...
    generate_startTime = time.time()
    coeff_path = await audio_to_coeff.generate(batch, save_dir, pose_style, ref_pose_coeff_path)
    generate_endTime = time.time()    
    execution_time = generate_endTime - generate_startTime
    ...
    if args.face3dvis:
        ...
        await gen_composed_video(args, device, first_coeff_path, coeff_path, audio_path, os.path.join(save_dir, '3dface.mp4'))
    generate_endTime = time.time()    
    execution_time = generate_endTime - generate_startTime
    ...
    #coeff2video
    generate_startTime = time.time()
    data = await get_facerender_data(coeff_path, crop_pic_path, first_coeff_path, audio_path, 
    ...
    generate_startTime = time.time()
    result = await animate_from_coeff.generate(data, save_dir, pic_path, crop_info, \
